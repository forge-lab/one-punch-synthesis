...
enum class Behavior
{
    NOTDEFINED, // default value, included in everything
    MAXPLUS, // max is increasing 
    MAXMINUS, //max is decreasing // 
    MINPLUS, // min is incr
    MINMINUS, //min is dec
};
...
public:
    ...
    int ID;
    bool isLine;
    Behavior ValChangeMAX;
    Behavior ValChangeMIN;
    Component(std::string N, Type O, std::vector<Type> I, int J) :Name(N), Output(O), Inputs(I), ID(J), isLine(true), ValChangeMAX(Behavior::NOTDEFINED),ValChangeMIN(Behavior::NOTDEFINED) {};
    Component(std::string N, Type O, std::vector<Type> I, int J, Behavior VCMIN, Behavior VCMAX) :Name(N), Output(O), Inputs(I), ID(J), isLine(true), ValChangeMAX(VCMAX), ValChangeMIN(VCMIN) {};
    Component(std::string N, Type O, std::vector<Type> I, int J, bool B) :Name(N), Output(O), Inputs(I), ID(J), isLine(B), ValChangeMAX(Behavior::NOTDEFINED), ValChangeMIN(Behavior::NOTDEFINED) {};
    Component(std::string N, Type O, std::vector<Type> I, int J, bool B,Behavior VCMIN, Behavior VCMAX) :Name(N), Output(O), Inputs(I), ID(J), isLine(B), ValChangeMAX(VCMAX), ValChangeMIN(VCMIN) {};
};
...
public:
    ...
    DSL(reader textinfo)
    {
       ...
       Components.push_back(Component("Sum", Type::INTEGER, std::vector<Type>{Type::LIST}, 3,Behavior::MINPLUS,Behavior::MAXPLUS)); 
      ...
      Components.push_back(Component("Drop", Type::LIST, std::vector<Type>{Type::LIST,Type::INTEGER}, 9));
        ...
        Components.push_back(Component("Minimum", Type::INTEGER, std::vector<Type>{Type::LIST}, 11,Behavior::NOTDEFINED,Behavior::MAXMINUS));
        Components.push_back(Component("Maximum", Type::INTEGER, std::vector<Type>{Type::LIST}, 12,Behavior::MINPLUS,Behavior::NOTDEFINED)); 
        
        
        //Functions for HOF Map
     
        
        Components.push_back(Component("AddOneLambda", Type::LAMBDA_INT, std::vector<Type>{}, 13,false,Behavior::MINPLUS,Behavior::MAXPLUS));
        Components.push_back(Component("SubOneLambda", Type::LAMBDA_INT, std::vector<Type>{}, 14,false,Behavior::MINMINUS,Behavior::MAXMINUS));
        ...
        Components.push_back(Component("Pow2Lambda", Type::LAMBDA_INT, std::vector<Type>{}, 21,false,Behavior::MINPLUS,Behavior::MAXPLUS));
